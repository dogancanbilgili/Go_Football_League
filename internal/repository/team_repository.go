...
func (r *teamRepo) GetAll() ([]models.Team, error) {
	...
	err := r.db.Select(&teams, "SELECT id, name, attack, midfield, defence FROM teams ORDER BY id")
	return teams, err
}

func (r *teamRepo) GetByID(id int) (*models.Team, error) {
	var team models.Team
	err := r.db.Get(&team, "SELECT id, name, attack, midfield, defence FROM teams WHERE id = $1", id)
	return &team, err
}
